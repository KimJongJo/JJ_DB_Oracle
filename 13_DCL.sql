...
-- GRANT RESOURCE, CONNECT TO exam_kjj;
-- 롤 (role)
...
-- 2. 새 연결 추가
--> ORA-01045: 사용자 KJJ_SAMPLE는 CREATE SESSION 권한을 가지고있지 않음; 로그온이 거절되었습니다
...
GRANT CREATE SESSION TO kjj_sample;
...
GRANT CREATE TABLE TO kjj_sample;


ALTER USER kjj_sample DEFAULT TABLESPACE
...
GRANT CONNECT, RESOURCE TO kjj_sample;
...
-- kh_kjj / kjj_sample 사용자 계정끼리 서로 객체 접근 권한 부여


-- 1. (sample) kh_kjj 계정의 EMPLOYEE 테이블 조회
...
GRANT SELECT ON EMPLOYEE TO kjj_sample;
...
-- 4. (kh) sample 계정에 부여한 EMPLOYEE 테이블 조회 권환 회수 (REVOKE)

-- [권한 회수 작성법]
-- REVOKE 객체권한 ON 객체명 FROM 사용자명;
REVOKE SELECT ON EMPLOYEE FROM kjj_sample;
