...
-- 6. 춘 기술대학교는 총장을 제외하고 모든 교수들이 소속 학과를 가지고 잇다. 그럼
...
-- 7. 혹시 전산상의 착오로 학과과 지정되어 있지 않은 학생이 있는지 확인하고자 한다.
...
FROM TB_STUDENT
WHERE SUBSTR(TO_CHAR(ENTRANCE_DATE), 0, 2) = '02' AND 
SUBSTR(STUDENT_ADDRESS, 0, 2) = '전주' AND ABSENCE_YN = 'N';



SELECT STUDENT_NO , STUDENT_NAME , STUDENT_SSN 
FROM TB_STUDENT
WHERE EXTRACT(YEAR FROM ENTRANCE_DATE) = 2002 AND
STUDENT_ADDRESS LIKE '전주%' AND ABSENCE_YN = 'N';
