...
-- 1. FROM절 : FROM 테이블명
-- 2. WHERE절 : WHERE 컬럼명 연산자 값; (조걸절)
-- SECELT * FROM EMPLOYEE WHERE EMP_ID = 300; 
...
-- EMPLOYEE 테이블에서 급여가 300만 이상이고 500만 미만인 사원의
-- 사번, 이름, 급여, 전화번호 조회
SELECT EMP_ID, EMP_NAME, SALARY, PHONE
FROM EMPLOYEE
...
-- WHERE EMP_NAME LIKE '전__'; 이름이 세글자인 경우
...
-- EMAIL 에서 _앞에 글자가 세 글자인 사원 이름, 이메일 조회
 ...
 * 1. FROM절
 * 2. WHERE절
 * 4. ORDER BY 컬럼명 | 별칭 | 컬럼 순서 [ACS(오름차순 : 기본값) | DESC(내림차순)]
 * 																						[NULLS FIRST(NULL 값 앞으로) | LAST(NULL 값 뒤로)]
 * */
