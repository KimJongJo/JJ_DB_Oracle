...
-- 그룹 함수 : N개의 값을 읽어 1개의 결과를 반환 (합계, 평군, 최대, 최소)
...
-- TRUNC(숫자 | 컬럼명 [, 위치]) : 특정 위치 아래를버림(절삭)
...
-- 2024-02-29 11:20:33.770 + 0900 (UTC에서 + 9시간 나타님, 한국 표준시)
...
-- ADD MONTHS(날짜, 숫자) : 날짜에 숫자만큼의 개월 수를 더함(음수도 가능)
...
SELECT SYSDATE FROM DUAL; -- 2024-02-29 12:21:04.000

-- 2024/02/29 12:21:04 목요일
...
--> "" 쌍따옴표를 이용해서 단순함 문자로 인식시키면 해결됨

----------------------------------------------------------------------

/* 날짜로 변환 TO_DATE */

-- TO_DTAE(문자형 데이터, [포맷]) : 문자형 데이터를 날짜로 변경
-- TO_DTAE(숫자형 데이터, [포맷]) : 숫자형 데이터를 날짜로 변경
--> 지정된 포맷으로 날짜를 인식함
...
-- ORA-08161 : 리터럴이 형식 문자열과 일치하지 않음
...
SELECT TO_DATE('400505', 'RRMMDD') FROM DUAL; -- -05-05 00:00:00.000
...
-- NVL(컬럼명, 컬럼값 NULL일 때 바꿀 값) : NULL인 컬럼값을 다른값으로 변경
...
-- SUM(숫자가 기록된 컬럼명) : 합계
-- 모든 직원의 급여 합
SELECT SUM(SALARY) "모든 직원의 급여합" -- 70096240
FROM EMPLOYEE;

-- AVG(숫자가 기록된 컬럼명) : 평균
-- 전 직원 급여 평균
SELECT ROUND(AVG(SALARY)) "모든 직원의 급여 평균" -- 3047663
FROM EMPLOYEE;
...
-- COUNT() : 행 개수를 헤아려서 리턴
-- COUNT([DITINCT] 컬럼명) : 중복을 제거한 행 개수를 헤아려서 리턴
