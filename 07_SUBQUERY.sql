 ...
 * 
 * - 다중행 다중열 서브쿼리 : 조회 겨로가 행 수와 열 수가 여러개 일 때
 * 
 * - 상(호연)관 서브쿼리 : 서브쿼리가 만든 결과 값을 메인쿼리가 비교연산할 때
 * 													메인 쿼리 테이블의 값이 변경되면 서브쿼리의 결과 값도 바뀌는 서브쿼리
 * 
 * - 스칼라 서브쿼리 : 상관 쿼리이면서 결과 값이 하나인 서브쿼리
 * 
 * ** 서브쿼리 유형에 따라 서브 쿼리 앞에 붙는 연산자가 다름 ** 
 * 
 * 
 * */							
...
-- 3-2) ANY를 이용하여 과장 중 가장 급여가 적은 직원의 급여를 초과하는 대리 조회
SELECT EMP_ID, EMP_NAME, JOB_NAME, SALARY
FROM EMPLOYEE
NATURAL JOIN JOB
WHERE JOB_NAME = '대리'
...
-- 3) EMPLOYEE 테이블에서 위의 결과와 동일한 사원을 구하시오.
SELECT EMP_NAME, DEPT_CODE
FROM EMPLOYEE
...
-- 메인 쿼리의 테이블값이 변경되면 서브쿼리의 결과값도 바뀌게 되는 구조
 ...
 * 
 * --> 서브쿼리 결과가 메인쿼리에 행들 중 일치하는게 있냐? TRUE
 * 		 --> 그럼 일치하는 행을 결과에 포함해준다!
 * 
 * 
 * */						
...
-- 부서별 입사일이 가장 빠른 사원의
-- 사번, 이름, 부서코드, 부서명(NULL이면 '소속없음'), 직급명, 입사일 조회 하고
-- 입사일이 빠른순으로 정렬하기
-- 단, 퇴사한 직원은 제외
						
-- 메인 쿼리 (사번, 이름, 부서명(NULL이면 '소속없음'), 직급명, 입사일
--																						단, 퇴사한 직원은 제외)
 ...
 *  216 차태연 D1 인사관리부 사원 2013-03-01 00:00:00.000
 *  217 전지연 D1 인사관리부 사원 2007-03-20 00:00:00.000
 * 
 * */
...
SELECT EMP_ID, EMP_NAME, PHONE, TO_CHAR(SALARY,'0000000') SALARY, JOB_NAME
FROM EMPLOYEE
NATURAL JOIN JOB
